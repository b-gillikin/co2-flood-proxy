...
def _append_csv_line(
    container_name: str, blob_name: str, columns: list[str], row: dict[str, str]
) -> None:
...
def main(timer: func.TimerRequest) -> None:
    ...
    container_name = (
        os.getenv("AIR_QUALITY_CONTAINER", DEFAULT_CONTAINER).strip() or DEFAULT_CONTAINER
    )
    blob_prefix = (
        os.getenv("AIR_QUALITY_BLOB_PREFIX", DEFAULT_BLOB_PREFIX).strip() or DEFAULT_BLOB_PREFIX
    )
